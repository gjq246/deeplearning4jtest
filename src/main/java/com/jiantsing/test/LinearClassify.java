...
//        rr.initialize(new FileSplit(new File("src/main/resources/classification/linear_data_train.csv")));
        ...
        double[][] evalPoints = new double[nPointsPerAxis*nPointsPerAxis][2];//10000*2，存储坐标抽上的每一个点
        int count = 0;
        for( int i=0; i<nPointsPerAxis; i++ ){
            for( int j=0; j<nPointsPerAxis; j++ ){
                double x = i * (xMax-xMin)/(nPointsPerAxis-1) + xMin;//平分后递增，这个地方会有一段重复值，重复计算
...
//                System.out.println(y);
        ...
        INDArray predictionsAtXYPoints = model.output(allXYPoints);//对坐标轴上的点进行评估预测
